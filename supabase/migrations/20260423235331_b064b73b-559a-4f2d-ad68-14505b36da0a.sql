-- Profiles table (display info for the signed-in user)
...
-- Workout data: one JSON document per user
...
CREATE POLICY "Users can view their own workout data"
  ON public.workout_data FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own workout data"
